from io import BytesIO

import pandas as pd
from datetime import datetime as dt
from datetime import timedelta as td 

import streamlit as st
import streamlit_authenticator as stauth

from configuracion import config
from configuracion import read_json_from_supabase

st.set_page_config(
    page_title="Narcisse " + config.NOMBRE_SUCURSAL,
    page_icon=":bar_chart:",
    layout="wide",
    initial_sidebar_state="expanded"
)

#* USER AUTHENTICATION
credenciales = read_json_from_supabase(config.BUCKET_GENERAL, config.CREDENCIALES_FILE_OUTPUT)
authenticator = stauth.Authenticate(
    credenciales,
    st.secrets["COOKIE_NAME"],
    st.secrets["COOKIE_KEY"],
    int(st.secrets["COOKIE_EXPIRY_DAYS"]),
)
name, authentication_status, username = authenticator.login()

if authentication_status is False:
    st.error('Username/password is incorrect')
elif authentication_status is None:
    st.warning('Please enter your username and password')
elif authentication_status:
    col1, col2 = st.columns([4,1])
    with col1:
        st.success('Bienvenido {}'.format(name))
    with col2:
        authenticator.logout('Logout', 'main')
    
    st.title("Pérdidas " + config.NOMBRE_SUCURSAL)

    #? ANALISIS DE DATOS
    data = config.supabase.table(config.TAB_INVENTARIO).select("*").execute().data
    df = pd.DataFrame(data)
    df_inv = df[['clave', 'producto', 'categoria', 'fecha_estatus', 'estatus']]

    # Convertimos la columna de fecha_estatus a datetime
    df_inv['fecha_estatus'] = pd.to_datetime(df_inv['fecha_estatus'])

    # Filtramos por perdidas
    df_inv = df_inv[~df_inv['estatus'].isin(['ESCANEADO', 'VENDIDO', 'BORRAR_BEBIDA'])]

    # Agregamos columna año
    df_inv['anio'] = df_inv['fecha_estatus'].dt.year

    #================ FILTRO POR AÑO =================#
    col1_1, col1_2 = st.columns(2)
    with col1_1:
        anios_disponibles = sorted(df_inv['anio'].unique())
        anio_seleccionado = st.selectbox('Selecciona un año:', options=anios_disponibles, index=0)
        df_inv = df_inv[df_inv['anio'] == anio_seleccionado]

        #================ FILTRO POR MES =================#
        df_inv['mes'] = df_inv['fecha_estatus'].dt.month
        df_inv['dia'] = df_inv['fecha_estatus'].dt.day

        meses = {
            1: 'Enero', 2: 'Febrero', 3: 'Marzo', 4: 'Abril',
            5: 'Mayo', 6: 'Junio', 7: 'Julio', 8: 'Agosto',
            9: 'Septiembre', 10: 'Octubre', 11: 'Noviembre', 12: 'Diciembre'
        }

        mes_seleccionado = st.selectbox(
            'Selecciona un mes:',
            options=list(meses.keys()),
            format_func=lambda x: meses[x],
            index=0
        )

        df_filtrado = df_inv[df_inv['mes'] == mes_seleccionado]

    with col1_2:
        #================ FILTRO POR DÍA =================#
        if df_filtrado.empty:
            st.warning(f"No hay datos disponibles para {meses[mes_seleccionado]} de {anio_seleccionado}.")
        else:
            # Obtener el rango de fechas mínimo y máximo disponibles para ese mes
            fecha_min = df_filtrado['fecha_estatus'].min().date()
            fecha_max = df_filtrado['fecha_estatus'].max().date()

            rango_fechas = st.date_input(
                "Filtrar por rango de fechas:",
                value=(fecha_min, fecha_max),
                min_value=fecha_min,
                max_value=fecha_max
            )

            # Asegurarnos de que rango_fechas tenga inicio y fin
            if isinstance(rango_fechas, tuple) and len(rango_fechas) == 2:
                fecha_inicio, fecha_fin = rango_fechas
                df_filtrado = df_filtrado[
                    (df_filtrado['fecha_estatus'].dt.date >= fecha_inicio) &
                    (df_filtrado['fecha_estatus'].dt.date <= fecha_fin)
                ]
           

    #================ RESTO DE FILTROS =================#
    if not df_filtrado.empty:
        col1_1, col1_2, col1_3 = st.columns(3)
        with col1_1:
            estatus_seleccionado = st.multiselect('Filtrar por estatus:', df_filtrado['estatus'].unique())
            if estatus_seleccionado:
                df_filtrado = df_filtrado[df_filtrado['estatus'].isin(estatus_seleccionado)]

        with col1_2:
            categoria_seleccionada = st.multiselect('Filtrar por categoría:', df_filtrado['categoria'].unique())
            if categoria_seleccionada:
                df_filtrado = df_filtrado[df_filtrado['categoria'].isin(categoria_seleccionada)]

        with col1_3:
            if categoria_seleccionada:
                productos_disponibles = df_filtrado['producto'].unique()
            else:
                productos_disponibles = df_filtrado['producto'].unique()

            producto_seleccionado = st.multiselect('Filtrar por producto:', productos_disponibles)
            if producto_seleccionado:
                df_filtrado = df_filtrado[df_filtrado['producto'].isin(producto_seleccionado)]

        #================ MÉTRICAS SOLO TRAS FILTRAR =================#
        mermados = df_filtrado[df_filtrado['estatus'] == "MERMADO"].shape[0]
        degustados = df_filtrado[df_filtrado['estatus'] == "DEGUSTADO"].shape[0]
        daniados = df_filtrado[df_filtrado['estatus'] == "DAÑADO"].shape[0]
        total = mermados + degustados + daniados

        colm1, colm2, colm3, colm4 = st.columns(4)
        colm1.metric("TOTAL PÉRDIDA", f"{total} Prods")
        colm2.metric("MERMADOS", f"{mermados} Prods")
        colm3.metric("DEGUSTADOS", f"{degustados} Prods")
        colm4.metric("DAÑADOS", f"{daniados} Prods")

        #================ DESCARGA =================#
        def to_excel(df):
            output = BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                df.to_excel(writer, index=False, sheet_name='Perdidas')
                writer._save()
            output.seek(0)
            return output

        excel_data = to_excel(df_filtrado)
        st.download_button(
            label="Descargar en formato Excel",
            data=excel_data,
            file_name='Perdidas.xlsx',
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )

        #================ TABLA =================#
        st.table(df_filtrado[['clave', 'producto', 'categoria', 'fecha_estatus', 'estatus']])
